...
describe("CategoryInMemoryRepository", () => {
  ...
  it("should sort by createdAt when sort param is null", async () => {
    ...
    const items = [
      ...
      Category.fake().aCategory().withName('TEST').withCreatedAt(new Date(createdAt.getTime() + 100)).build(),
      Category.fake().aCategory().withName('fake').withCreatedAt(new Date(createdAt.getTime() + 200)).build(),
    ];
