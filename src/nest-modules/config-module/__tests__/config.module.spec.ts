...
describe('ConfigModule Unit Tests', () => {
  // Teste reescrito para usar async/await e rejects.toThrow
  it('should throw an error when env vars are invalid', async () => {
    // Espera que a promessa retornada por compile() seja rejeitada com um erro
    await expect(
      Test.createTestingModule({
        imports: [
          ConfigModule.forRoot({
            envFilePath: join(__dirname, '.env.fake'), // Usa o arquivo .env inválido
          }),
        ],
      }).compile(), // Chama compile() que dispara a validação
    ).rejects.toThrow(
